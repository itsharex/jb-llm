...
/* 21 */     Intrinsics.checkNotNullParameter(key, "key"); Intrinsics.checkNotNullParameter(values, "values"); Intrinsics.checkNotNullExpressionValue(key.toLowerCase(Locale.ROOT), "this as java.lang.String).toLowerCase(Locale.ROOT)"); this.values.put(key.toLowerCase(Locale.ROOT), ArraysKt.toMutableList((Object[])values));
/*    */   }
/*    */   
/*    */   public final void add(@NotNull String key, @NotNull String value) {
/* 25 */     Intrinsics.checkNotNullParameter(key, "key"); Intrinsics.checkNotNullParameter(value, "value"); HashMap<String, List<String>> hashMap = this.values; Intrinsics.checkNotNullExpressionValue(key.toLowerCase(Locale.ROOT), "this as java.lang.String).toLowerCase(Locale.ROOT)"); String str = key.toLowerCase(Locale.ROOT); int $i$f$getOrPut = 0;
/*    */ 
/*    */ 
/*    */ 
/*    */ 
...
/*    */     Intrinsics.checkNotNullExpressionValue(key.toLowerCase(Locale.ROOT), "this as java.lang.String).toLowerCase(Locale.ROOT)");
/*    */     this.values.remove(key.toLowerCase(Locale.ROOT)); }
/*    */   @Nullable public final String[] get(@NotNull String key) { Intrinsics.checkNotNullParameter(key, "key");
/*    */     Intrinsics.checkNotNullExpressionValue(key.toLowerCase(Locale.ROOT), "this as java.lang.String).toLowerCase(Locale.ROOT)");
/*    */     Collection $this$toTypedArray$iv = this.values.get(key.toLowerCase(Locale.ROOT));
/*    */     int $i$f$toTypedArray = 0;
/* 84 */     Collection thisCollection$iv = $this$toTypedArray$iv; (List)this.values.get(key.toLowerCase(Locale.ROOT));
/* 85 */     return ((List)this.values.get(key.toLowerCase(Locale.ROOT)) != null) ? (String[])thisCollection$iv.toArray((Object[])new String[0]) : null; } @NotNull public final String[] getKeys() { Intrinsics.checkNotNullExpressionValue(this.values.keySet(), "this.values.keys"); Collection<String> $this$toTypedArray$iv = this.values.keySet(); int $i$f$toTypedArray = 0;
/* 86 */     Collection<String> thisCollection$iv = $this$toTypedArray$iv;
/* 87 */     return thisCollection$iv.<String>toArray(new String[0]); }
/*    */ 
/*    */ }


/* Location:              /Volumes/Macintosh HD/Users/phodal/Desktop/ml-llm.jar!/ai/grazie/client/common/model/HeaderCollection.class
